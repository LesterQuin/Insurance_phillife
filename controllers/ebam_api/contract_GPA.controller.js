...
export const getGpaData = async (req, res) => {
    try {
        ...
        const isGpa = basicPlanName.includes('ACCIDENTAL DEATH') || basicPlanName.includes('GADDP') || basicPlanName.includes('GPA') || basicPlanName.includes('G-ADD') || basicPlanName.includes('GADD');

        if (!isGpa) {
            return error(res, `Invalid plan type for GPA GADDP. The plan for this application is: ${application.appData.basic_plan_name || 'unknown'}.`, 400);
        }

        const data = await EbamModel.getGpaData(id);
        ...
        return success(res, data, "GPA data fetched successfully.", 200);
    } catch (err) {
        console.error("Get GPA Data Error:", err);
        return error(res, err.message, 500);
    }
};

// Save/Update unified GPA data compiled together
export const saveGpaData = async (req, res) => {
    try {
        ...
        const savedData = await EbamModel.saveGpaData(id, req.body);
        ...
        return success(res, savedData, "GPA data updated successfully.", 200);
    } catch (err) {
        console.error("Save GPA Data Error:", err);
        return error(res, err.message, 500);
    }
};
