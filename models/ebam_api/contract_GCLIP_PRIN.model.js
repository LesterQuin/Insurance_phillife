...
export const ensureUnderwritingProvisionsTable = async () => {
    ...
    await pool.request().query(`
        IF OBJECT_ID('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'U') IS NULL
        BEGIN
            CREATE TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting (
                ...
                provision_face_amount NVARCHAR(MAX) NULL,
                provision_premium_computation NVARCHAR(MAX) NULL,
                provision_non_coverage NVARCHAR(MAX) NULL,
        ...
        ELSE
        BEGIN
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'eligible_individuals') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD eligible_individuals NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'participation_percentage') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD participation_percentage NVARCHAR(50) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'participation_minimum_no') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD participation_minimum_no NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'termination_age') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD termination_age INT NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'provision_enrollment') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD provision_enrollment NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'provision_rollover') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD provision_rollover NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'provision_termination') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD provision_termination NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'provision_definitions') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD provision_definitions NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'provision_claims') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD provision_claims NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'refund_of_premiums') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD refund_of_premiums NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'amount_of_insurance') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD amount_of_insurance NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'coverage_period') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD coverage_period NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'due_dates') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD due_dates NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'provision_face_amount') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD provision_face_amount NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'provision_premium_computation') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
                ADD provision_premium_computation NVARCHAR(MAX) NULL;
            END
            IF COL_LENGTH('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting', 'provision_non_coverage') IS NULL
            BEGIN
                ALTER TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
...
export const getUnderwritingProvisions = async (applicationId) => {
    ...
    const result = await pool.request()
        ...
        .query(`
            ...
            FROM DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting
    ...
    if (row) {
        return {
            ...
            provision_face_amount: row.provision_face_amount,
            provision_premium_computation: row.provision_premium_computation,
            provision_non_coverage: row.provision_non_coverage
        };
    }
    return null;
};
...
export const saveSpecialProvisions = async (applicationId, params = {}) => {
    ...
    const merged = {
        ...
        provision_face_amount: params.face_amount_insurance !== undefined ? params.face_amount_insurance : existing.provision_face_amount,
        provision_premium_computation: params.premium_computation !== undefined ? params.premium_computation : existing.provision_premium_computation,
        provision_non_coverage: params.non_coverage_provision !== undefined ? params.non_coverage_provision : existing.provision_non_coverage
    };
    ...
    await pool.request()
        ...
        .input('provision_face_amount', sql.NVarChar(sql.MAX), merged.provision_face_amount)
        .input('provision_premium_computation', sql.NVarChar(sql.MAX), merged.provision_premium_computation)
        .input('provision_non_coverage', sql.NVarChar(sql.MAX), merged.provision_non_coverage)
        .query(`
            MERGE INTO DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting WITH (HOLDLOCK) AS target
            USING (SELECT @application_id AS application_id) AS source
            ON target.application_id = source.application_id
            WHEN MATCHED THEN
                UPDATE SET 
                    ...
                    provision_face_amount = @provision_face_amount,
                    provision_premium_computation = @provision_premium_computation,
                    provision_non_coverage = @provision_non_coverage,
                    updated_at = GETDATE()
            WHEN NOT MATCHED THEN
                INSERT (application_id, provision_enrollment, provision_rollover, provision_termination, provision_definitions, provision_claims, provision_face_amount, provision_premium_computation, provision_non_coverage, created_at, updated_at)
                VALUES (@application_id, @provision_enrollment, @provision_rollover, @provision_termination, @provision_definitions, @provision_claims, @provision_face_amount, @provision_premium_computation, @provision_non_coverage, GETDATE(), GETDATE());
        `);
...
export const ensureUnderwritingLimitsTable = async () => {
    const pool = await poolPromise;
    await pool.request().query(`
        IF OBJECT_ID('DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting_limits', 'U') IS NULL
        BEGIN
            CREATE TABLE DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting_limits (
...
export const getUnderwritingLimits = async (applicationId) => {
    ...
    const result = await pool.request()
        ...
        .query(`
            ...
            FROM DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting_limits
            WHERE application_id = @application_id
        `);
...
export const saveUnderwritingLimits = async (applicationId, params = {}) => {
    ...
    await pool.request()
        ...
        .query(`
            MERGE INTO DHUB_UAT.sg.financial_insurance_gcli_principal_underwriting_limits WITH (HOLDLOCK) AS target
            USING (SELECT @application_id AS application_id) AS source
            ON target.application_id = source.application_id
            WHEN MATCHED THEN
                UPDATE SET 
