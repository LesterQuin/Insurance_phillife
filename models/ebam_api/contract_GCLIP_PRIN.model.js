...
// Ensure table for tracking custom Underwriting Limits exists
...
export const getGclipPrincipalData = async (applicationId) => {
    ...
    return {
        ...
        gclip_special_provision: {
            ...
            face_amount_insurance: provRow?.provision_face_amount || "",
            premium_computation: provRow?.provision_premium_computation || "",
            claims_procedure: provRow?.provision_claims || "",
            non_coverage_provision: provRow?.provision_non_coverage || ""
        },
...
// Save/Update unified GCLIP Principal data compiled together (with Transaction)
export const saveGclipPrincipalData = async (applicationId, data) => {
    await ensureUnderwritingProvisionsTable();
    await ensureUnderwritingLimitsTable();
    ...
    try {
        ...
        const faceAmountInsurance = getVal(data.gclip_special_provision?.face_amount_insurance, existingProv?.provision_face_amount);
        ...
        provReq.input('provision_face_amount', sql.NVarChar(sql.MAX), faceAmountInsurance);
        ...
        return await getGclipPrincipalData(applicationId);
    } catch (err) {
