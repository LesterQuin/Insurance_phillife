        ...
        <table class="compact-table" style="width: 100%; border-collapse: collapse; font-size: 9pt; margin-bottom: 15px;">
          ...
          <tbody>
            ${rows.map(row => {
              let rowTotal = 0;
              columns.forEach(col => {
                const freeRiderId = application.plan_id === 4 ? 27 : 7;
                if (Number(col.id) === freeRiderId) return;
              ...
              return `
                <tr>
                  <td style="padding: 6px; border: 1px solid #ddd; text-align: left; font-weight: bold; font-size: 8.5pt;">${row.label}</td>
                  ${cells.join('')}
                  ${isLastChunk ? `<td style="padding: 6px; border: 1px solid #ddd; text-align: center; font-weight: bold; background-color: #fafafa; font-size: 8.5pt;">${formatNumber(rowTotal)}</td>` : ''}
                </tr>
              `;
            }).join('')}
          </tbody>
